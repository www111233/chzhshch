...
class ChanFeature:
    ...
    def RSQ(self) -> float:
        ...
        """
        import numpy as np
        close = [ck.end for ck in self.cklines]
    ...
    def toPillar(self) -> ChanCandle:
        ...
        if self.direction is Direction.Up:
            return ChanCandle("Pillar", self.start.low, self.end.high, [])
        if self.direction is Direction.Down:
            return ChanCandle("Pillar", self.start.high, self.end.low, [])

    def isNext(self, o: "ChanFeature") -> bool:
        if self.end is o.start:
            return True
        return False
...
@enforce_types
class ChanZhongShu:
    __slots__ = ["symbol", "elements", "ok", "done", "__level", "__l", "__m", "__r"]
    def __init__(self, symbol:str, left:[ChanCandle, ChanFeature], mid:[ChanCandle, ChanFeature], right:[ChanCandle, ChanFeature], level:int=0):
        ...
        for feature in [left, mid, right]:
            if doubleRelation(self.interval, feature.toPillar()) in (Direction.JumpUp, Direction.JumpDown):
                self.ok = False
                #raise ChanException("不是中枢")
                break
...
def handleInclude(l:ChanCandle, r:ChanCandle, raw:RawCandle):
    ...
    direction = doubleRelation(l, r)
    if not hasInclude(r, raw):
        return False, raw.toChanCandle()
    return True, removeInclude(r, raw, direction)
...
class BiHandler:
    ...
    def __replace_bi(self, fx:ChanFenXing, n:int):
        ...
        if old and old.end.style != fx.style:raise ChanException
    ...
    def __append_bi(self, fx: ChanFenXing, n:int):
        if fx.style in (Shape.G, Shape.D):
            ...
            if len(self.points) >= 2:
                ...
                if self.zsh:
                    self.zsh.append(bi)
                #print(self.zsMachine.state, len(self.zhshs), bi, not self.zshCache[0], not self.zshCache[1], not self.zshCache[2])
                return bi
    ...
    def __pop_bi(self, n:int):
        if self.points:
            ...
            if self.features:
                ...
                # 开始 处理中枢
                #print("pop", self.zsMachine.state, len(self.zhshs), tz, not self.zshCache[0], not self.zshCache[1], not self.zshCache[2])
                if self.zsh:
                    t = self.zsh.pop(tz, sys._getframe().f_lineno)
                # 结束 中枢处理

                #tz.clear(sys._getframe().f_lineno)

            if len(self.features) >= 2:
                if self.features[-2].end != self.features[-1].start:
                    raise ValueError("pop 无法衔接")
            self.log("line:", n, colored("__pop_bi is called", "green"), fx)
            self.log("line:", n, colored("__pop_bi is called", "green"), tz)
            return fx, tz
    ...
    @enforce_types
    def isLow(self, l: ChanCandle, r: ChanCandle):
        low = min(self.cklines[l.index:r.index+1], key=lambda x:x.low).low
        if low == r.low:return True

    @enforce_types
    def isHigh(self, l: ChanCandle, r: ChanCandle):
        high = max(self.cklines[l.index:r.index+1], key=lambda x:x.high).high
        if high == r.high:return True

    @enforce_types
    def isBi(self, start: ChanCandle, end: ChanCandle, mode=None, offset=5, isNew=1):
        if start.dt > end.dt:return -1
        relation = doubleRelation(start, end)
        if not (relation in (Direction.Up, Direction.JumpUp, Direction.Down, Direction.JumpDown)):
            return 8
        bi = self.cklines[start.index: end.index+1]
        size = len(bi)

        if isNew:
            offset = 4
            realSize = sum([len(k.elements) for k in bi])
            if realSize <= 4:
                ...
                return 2
    ...
    @enforce_types
    def problem(self, start: ChanCandle, end: ChanCandle, mode=None, offset=5, isNew=1):
        t = self.isBi(start, end, mode, offset, isNew)
        if t:return t
        relation = doubleRelation(start, end)

        if relation in (Direction.Up, Direction.JumpUp):
            if not self.isHigh(start, end):return 5

        elif relation in (Direction.Down, Direction.JumpDown):
            if not self.isLow(start, end):return 7
    ...
    def checkPoints(self, fx):
        ...
        if not (self.points[-1].style is states[self.biMachine.state]):
            raise ChanException(self.state, "状态不符", self.points[-1].style, "last style:", fx.style)
    ...
    def append(self, fx: ChanFenXing):
        ...
        if not self.points:
            if fx.style is Shape.D:
                self.biMachine.state  = 1
                self.__append_bi(fx, sys._getframe().f_lineno)
            elif fx.style is Shape.G:
                self.biMachine.state  = -1
                self.__append_bi(fx, sys._getframe().f_lineno)
            else:
                print(self.__class__.__name__, "未知类型", fx.style)
            print("@@@@初始化@@@@")
            self.stateStack.push(self.biMachine.state)
            return self

        if fx in self.points[-10:]:
            print("bi 存在退出",len(self.points), fx)
            return self

        self.log("line:", sys._getframe().f_lineno, "APPEND", self.points[-1].style if self.points else None, fx.style, self.state, fx.m)
        ...
        while 1:
            if self.points[-1].m is fx.m:
                if self.points[-1].style == fx.style:
                    ...
                    self.stateStack.pop()
                    self.biMachine.state = self.stateStack.gettop()
                    state = self.stateStack.gettop()

                #raise ChanException("什么情况， 无法替换")

            if fx.style is Shape.D: # 底分型
                if self.biMachine.state == 0:
                    self.__append_bi(fx, sys._getframe().f_lineno)
                    state = -2

                elif self.biMachine.state == -1:
                    if not self.problem(self.points[-1].m, fx.m, isNew=isNew):
                        if len(self.points) > 1:
                            if fx.low < self.points[-2].low:
                                self.B = fx
                                self.log("设置 4")
                                info = info + "设置4"
                        self.__append_bi(fx, sys._getframe().f_lineno)
                        state = -2
                        self.log(colored("转换状态, 少阴至太阴", "red"))

                elif self.biMachine.state == 1:
                    if fx.low < self.points[-1].low:
                        self.log(colored("非正常替换 1", "red"))
                        self.__replace_bi(fx, sys._getframe().f_lineno)
                        state = -2

                elif self.biMachine.state == -2:
                    if fx.low <= self.points[-1].low:
                        if self.T and SX:
                            if self.points[-2].dt < self.T.dt < fx.dt:
                                self.__pop_bi(sys._getframe().f_lineno)
                                self.__pop_bi(sys._getframe().f_lineno)
                                self.__append_bi(self.T, sys._getframe().f_lineno)
                                state = 2
                                break
                        self.__replace_bi(fx, sys._getframe().f_lineno)
                        state = -2
                        self.log(colored("刷新状态, 太阴", "red"))
                    #else:
                    #    if 0 == self.isBi(self.points[-1].m, fx.r, isNew=isNew):
                    #        state = 1
                        ...
                        if len(self.points) > 1:
                            if fx.low < self.points[-2].low:
                                if fx.r.high > self.points[-1].high:
                                    if SX and tmp and fx.dt > tmp.dt > self.points[-2].dt and tmp.high >= self.points[-2].high:
                                        self.__pop_bi(sys._getframe().f_lineno)
                                        self.__pop_bi(sys._getframe().f_lineno)
                                    ...
                                    if fx.style is self.points[-2].style:
                                        if tmp and tmp.dt < self.points[-2].dt:
                                            tmp = None
                                        if tmp:
                                            if fx.style is tmp.style:
                                                if fx.low < tmp.low:
                                                    self.B = fx
                                                    self.log("刷新 设置 1")
                                                    info = info + "刷新设置1"
                                        ...
                                        raise Exception("分型错误")

                else:
                    raise ChanException("未知状态", self.state, self.points)
                break

            elif fx.style is Shape.G: # 顶分型
                if self.biMachine.state == 0:
                    self.__append_bi(fx, sys._getframe().f_lineno)
                    state = 2

                elif self.biMachine.state == -1:
                    if fx.high > self.points[-1].high:
                        self.log(colored("非正常替换 2", "red"))
                        self.__replace_bi(fx, sys._getframe().f_lineno)
                        state = 2
                        break
                    ...
                    if not self.problem(self.points[-1].m, fx.m, isNew=isNew):
                        if len(self.points) > 1:
                            if fx.high > self.points[-2].high:
                                self.T = fx
                                self.log("设置 3")
                                info = info + "设置3"
                        self.__append_bi(fx, sys._getframe().f_lineno)
                        state = 2
                        self.log(colored("转换状态, 少阳至太阳", "red"))
                        break
                    ...
                    if fx.high >= self.points[-1].high:
                        if self.B and SX:
                            if self.points[-2].dt < self.B.dt < fx.dt:
                                self.__pop_bi(sys._getframe().f_lineno)
                                self.__pop_bi(sys._getframe().f_lineno)
                                self.__append_bi(self.B, sys._getframe().f_lineno)
                                state = -2
                                break
                        self.__replace_bi(fx, sys._getframe().f_lineno)
                        state = 2
                        self.log(colored("刷新状态, 太阳", "red"))
                    #else:
                    #    if 0 == self.isBi(self.points[-1].m, fx.r, isNew=isNew):
                    #        state = -1

                else:
                    raise ChanException("未知状态", self.state)
                break

            # 上涨， 下跌
            elif fx.style is Shape.S: # 上升分型
                if self.biMachine.state == 1:
                    ...
                    ...
                    if fx.r.high > self.points[-1].high:
                        self.__pop_bi(sys._getframe().f_lineno)
                        state = 1
                        self.log(colored("从 少阴 到 少阳。", "red"))
                    ...
                    if fx.r.high > self.points[-1].high:
                        if self.B and SX:
                            if self.points[-2].dt < self.B.dt < fx.dt:
                                ...
                                self.__pop_bi(sys._getframe().f_lineno)
                                self.__append_bi(self.B, sys._getframe().f_lineno)
                                if len(self.cklines[self.points[-1].m.index:fx.m.index+1]) >= 5:
                                    state = 1
                                else:
                                    state = -2
                                break
                        self.__pop_bi(sys._getframe().f_lineno)
                        state = 1
                        self.log(colored("转换状态, 太阳至少阳", "red"))

                elif self.biMachine.state == -2:
                    if not self.problem(self.points[-1].m, fx.r, isNew=isNew):
                        state = 1
                        self.log(colored("转换状态, 太阴至少阳", "red"))
                        break
                    else:
                        if len(self.points) > 1 and fx.high > self.points[-2].high:
                            info = "上涨设置"
                            fx.r.info = self.state + "::" + info

                else:
                    raise ChanException("未知状态", self.state)

            elif fx.style is Shape.X: # 下降分型
                if self.biMachine.state == 1:
                    if fx.r.low < self.points[-1].low:
                        self.__pop_bi(sys._getframe().f_lineno)
                        state = -1
                        self.log(colored("从 少阳 到 少阴。", "red"))
                    ...
                    if not self.problem(self.points[-1].m, fx.r, isNew=isNew):
                        state = -1
                        self.log(colored("转换状态, 太阳至少阴", "red"))
                    else:
                        if len(self.points) > 1 and fx.low < self.points[-2].low:
                            info = "下跌设置1"
                            fx.r.info = self.state + "::" + info
                ...
                print("*   ### unknow 分型", fx.style, "###   *")
            break

        self.biMachine.state = state
        fx.m.info = info + "::" + self.state
        #print(" ", self.appended, self.poped)
        if self.stateStack.gettop()!=state:self.stateStack.push(self.biMachine.state)
...
class ZhongShuHandler(FeatureMachine):
    ...
    @property
    def state(self):
        return {0: "混沌", 1: "阳", -1: "阴"}[self.zsMachine.state]
...
class FeatureHandler(FeatureMachine):
    ''' 线段处理 '''
    __slots__ = ["visual", "last", "stack", "__level", "zsh", "featureMachine", "features", "segments", "points", "lastFeature", "lastMerge", "debug", "isVisual"]
...
class ChZhShCh:
    def __init__(self):
        ...
        self.logging = logging("chzhshch2")
        self.logging.outstream = open("chzhshch2.log", "w")
        ...
        self.bzsHandler.logging = self.logging
        self.biHandler.logging = self.logging
        self.biHandler.zsh = self.bzsHandler
        #self.biHandler.debug = 1

        self.featureHandler = FeatureHandler(1)
        self.featureHandler.logging = self.logging
...
@timeit
def test2():
    with open("/storage/emulated/0/Moscow/Lite/cl/ltc1633785266", "rb") as f:
        dat = f.read()
        czsc = ChZhShCh()
        i = 1
        while dat:
